...
공식에서 ls는 Laplace smoothing기법으로 확률이 0이 되는 것을 방지하기 위해 1을 더해주는 것이다. 여기서 정확히는 1exp-8값을 사용한다. 그리고 G는 대각행렬의 합을 의미한다.  
이로써 학습 속도를 올린다.  
...
# 기본적인 딥러닝에서 좋은 결과를 얻기 위해서는 1순위 손실함수 2순위 활성화함수 3순위 최적화함수 순으로 중요하다. 
손실함수에서 예측이 얼마나 정답과 다른지를 파악해야 되므로 가장 중요하다. 일반적인 것을 써도 되나 문제의 상황에 맞게 만들어쓰는게 당연히 좋다.  
다음으로 예측과 정답을 같게 만들어주는 수정 과정(손실함수의 값을 줄이는)이 필요한데 그것이 최적화함수 이다.  
마지막으로 그러한 수정과정을 언제 적ㅈ
물론 다양한 환경에 맞는 딥러닝 모델 마다 중요시하는 parameter는 그때 그때 다를 수 있다.  
여기서 말하는 것은 가장 기본적인 딥러닝 즉 어떤 딥러닝이라도 안 중요시 여기지는 않는 parameter를 말한다. (오해가 없기를)  
