...
공식은 ReLu = max(0,x) 이다.  
x(입력)가 0보다 크면 기울기가 1인 직선, 0보다 작으면 함수 값이 0이 된다. 이는 0보다 작은 값들에서 뉴런이 죽을 수 있는 단점이 있긴하다.  
sigmoid는 입력에 아주 큰 음수가 들어와도 가중치 업데이트가 계속 진행되는데 이와 달리 ReLu는 음수에서 0이 곱해서 음수 가중치들을 다 제거 해버린다.  
이는 
그래서 0이하의 값들을 살리기 위해 다양한 방법들이 나온다. 그중 하나인 Leaky ReLu = max(0.01x,x)이다.  
